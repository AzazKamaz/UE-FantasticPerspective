...
namespace MoviePipeline
{
	...
	void FFantasticPerspectiveDeferredRenderEnginePass::Setup(TWeakObjectPtr<UMoviePipeline> InOwningPipeline, const FMoviePipelineRenderPassInitSettings& InInitSettings)
	{
	...
	void FFantasticPerspectiveDeferredRenderEnginePass::Teardown()
	{
	...
	void FFantasticPerspectiveDeferredRenderEnginePass::AddReferencedObjects(FReferenceCollector& Collector)
	{
		FSceneViewStateInterface* Ref = ViewState.GetReference();
		if (Ref)
		{
	...
	void FFantasticPerspectiveDeferredRenderEnginePass::RenderSample_GameThread(const FMoviePipelineRenderPassMetrics& InSampleState)
	{
	...
	FSceneView* FFantasticPerspectiveDeferredRenderEnginePass::CalcSceneView(FSceneViewFamily* ViewFamily, const FMoviePipelineRenderPassMetrics& InSampleState)
	{
		...
		ApplyFantasticPerspective(ViewInitOptions);
	...
	void FFantasticPerspectiveDeferredRenderEnginePass::ApplyFantasticPerspective(FSceneViewProjectionData& ProjectionData)
	{
		APlayerController* PlayerController = GetPipeline()->GetWorld()->GetFirstPlayerController();
		FSceneViewProjectionData InProjectionData = ProjectionData;

		if (PlayerController)
			if (auto viewTarget = PlayerController->GetViewTarget()) {
				TArray<UActorComponent*> found;

				viewTarget->GetComponents(UFantasticPerspectiveComponent::StaticClass(), found);

				for (auto i : found)
					if (Cast<UFantasticPerspectiveComponent>(i)->CachedApply(PlayerController, EFantasticPerspectiveStereoscopicPass::eFPSSP_FULL, InProjectionData.ViewOrigin, InProjectionData.ViewRotationMatrix, InProjectionData.ProjectionMatrix, ProjectionData.ViewOrigin, ProjectionData.ViewRotationMatrix, ProjectionData.ProjectionMatrix))
						return;
			}
		...
		if (PlayerController) {
			...
			PlayerController->GetComponents(UFantasticPerspectiveComponent::StaticClass(), found);
		...
		TArray<AActor*> found;
		UGameplayStatics::GetAllActorsOfClass(PlayerController->GetWorld(), AFantasticPerspectiveActor::StaticClass(), found);
		for (auto i : found)
			if (Cast<AFantasticPerspectiveActor>(i)->CachedApply(PlayerController, EFantasticPerspectiveStereoscopicPass::eFPSSP_FULL, InProjectionData.ViewOrigin, InProjectionData.ViewRotationMatrix, InProjectionData.ProjectionMatrix, ProjectionData.ViewOrigin, ProjectionData.ViewRotationMatrix, ProjectionData.ProjectionMatrix))
				return;

		ProjectionData = InProjectionData;
	}
}

void UFantasticPerspectiveDeferredPassBase::GetRequiredEnginePassesImpl(TSet<FMoviePipelinePassIdentifier>& RequiredEnginePasses)
{
	Super::GetRequiredEnginePassesImpl(RequiredEnginePasses);

	RequiredEnginePasses.Add(FMoviePipelinePassIdentifier(TEXT("MainFantasticPerspectiveDeferredPass")));
}

void UFantasticPerspectiveDeferredPassBase::SetupImpl(TArray<TSharedPtr<MoviePipeline::FMoviePipelineEnginePass>>& InEnginePasses, const MoviePipeline::FMoviePipelineRenderPassInitSettings& InPassInitSettings)
{
	...
	const FMoviePipelinePassIdentifier DesiredPass = FMoviePipelinePassIdentifier(TEXT("MainFantasticPerspectiveDeferredPass"));
	for (const TSharedPtr<MoviePipeline::FMoviePipelineEnginePass>& Pass : InEnginePasses)
	{
		if (Pass->PassIdentifier == DesiredPass)
		{
			...
			TSharedPtr<MoviePipeline::FFantasticPerspectiveDeferredRenderEnginePass> DeferredPass = StaticCastSharedPtr<MoviePipeline::FFantasticPerspectiveDeferredRenderEnginePass>(Pass);
			DeferredPass->BackbufferReadyDelegate.AddUObject(this, &UFantasticPerspectiveDeferredPassBase::OnBackbufferSampleReady);
			DeferredPass->SetupViewDelegate.AddUObject(this, &UFantasticPerspectiveDeferredPassBase::OnSetupView);
...
void UFantasticPerspectiveDeferredPassBase::GatherOutputPassesImpl(TArray<FMoviePipelinePassIdentifier>& ExpectedRenderPasses)
{
...
void UFantasticPerspectiveDeferredPassBase::OnBackbufferSampleReady(TArray<FFloat16Color>& InPixelData, FMoviePipelineRenderPassMetrics InSampleState)
{
...
void UFantasticPerspectiveDeferredPassBase::OnSetupView(FSceneViewFamily& InViewFamily, FSceneView& InView, const FMoviePipelineRenderPassMetrics& InSampleState)
{
	struct FIterator
	{
